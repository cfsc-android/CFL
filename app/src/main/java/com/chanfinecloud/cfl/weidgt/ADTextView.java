...
import com.chanfinecloud.cfl.util.SizeUtils;
...
public class ADTextView extends View {
    private ArrayList<NoticeEntity> data;//用于展示的数据
    ...
    private void obtainStyledAttrs(AttributeSet attrs) {
        ...
        HeadColor = array.getColor(R.styleable.ADTextView_ad_text_front_color, Color.BLACK);
    ...
    protected void onDraw(Canvas canvas) {
        if (data != null && data.size() > 0) {
            //当前需要绘制的文字内容
            head = data.get(index).getTitle();
            content = data.get(index).getSummary();
            //头部文字的测量
    ...
    public ArrayList<NoticeEntity> getData() {
        return data;
    }
...
//                CustomToast.showToast(getContext(),"暂无数据");
                Utils.showPrompt("暂无数据");
            }
            return true;
        } else {
